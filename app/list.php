				...
				if (!empty($_SESSION['success_message'])) {
					...
					foreach ($data as $key => $value) {		
						...
						echo '<div class="card card-inverse card-info mb-3" style="padding:1%;">					
								  ...
								  echo '<a class="btn btn-warning float-right" href="update.php?id='.$value['id'].'">Edit</a>';
								  // Add the delete button
								  echo '<form action="delete.php" method="post" style="display: inline-block;">
											<input type="hidden" name="property_id" value="'.$value['id'].'">
											<button type="submit" class="btn btn-danger float-right">Delete</button>
										</form>';
								  echo '<div class="clearfix"></div>'; // Clear float
